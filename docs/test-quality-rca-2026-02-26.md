...
**Why分析:**
1. テストは書いたが実行環境（AWS/Slack認証）が整備されていない
2. CI/CDパイプラインが未構築 → ローカル手動実行のみ
3. ローカルでも認証情報をセットアップしていない → 書いて終わり
4. **根本: テスト「作成」がゴールで「実行」がゴールではなかった**
...
**Why分析:**
1. requirements.mdにセキュリティテスト要件が明記されていない
2. Kiroが「正常系テスト」を優先して書き、攻撃パターンを後回しにした
3. AYAのACレビューでセキュリティ観点が欠如していた
4. **根本: requirements.mdのACに「セキュリティテスト」項目がない**
...
**Why分析:**
1. 最初は「E2Eテスト」として設計されたが、認証環境がなくMockに置き換えた
2. ファイル名を変更しなかった（mock化後もE2Eのまま）
3. AYAが「テスト名=テスト内容」と信頼してファイル内容を検証しなかった
4. **根本: テストの「名前」と「実態」の乖離を検知する仕組みがない**
...
| # | 提案 | 効果 | 工数 |
|---|---|---|---|
| 1 | テスト名とmock使用率の自動チェックスクリプト | 名前詐欺検知 | 小 |
| 2 | `pytest-cov` でカバレッジ実測 + CI組み込み | 実カバレッジ可視化 | 小 |
| 3 | セキュリティテスト追加（test_security.py） | 攻撃パターン検証 | 中 |
| 4 | GitHub Actions CI/CD構築（E2Eジョブ分離） | skipテスト実行 | 中 |
| 5 | Phase 1.5 TDD導入（テスト先行） | 構造的mock偏重防止 | 大 |
| 6 | テスト品質ゲート追加（mock/real比率閾値） | 品質劣化防止 | 小 |
