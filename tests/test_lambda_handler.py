...
def test_get_secrets__no_arn__returns_stub(monkeypatch):
    ...
    import importlib
    import yui.lambda_handler as lh
    lh._secrets_cache = None  # キャッシュリセット

    secrets = lh._get_secrets()
    assert "BEDROCK_MODEL_ID" in secrets
    lh._secrets_cache = None  # 後処理
...
def test_verify_slack_signature__no_signing_secret__returns_true(monkeypatch):
    ...
    from yui.lambda_handler import _verify_slack_signature
