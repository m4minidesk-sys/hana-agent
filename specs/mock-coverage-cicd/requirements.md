...
| ID | 要求 | 受入基準 |
|---|---|---|
| FR-08-A1 | Lambda handlerのリクエストパース+レスポンス生成テスト | Slack Events APIのchallenge/event/url_verification 3パターン以上。stubされたboto3/Slack SDKでhandler単体テスト。url_verification challenge応答が同期的に返却されること（Bedrock呼び出しなし、レスポンス時間≤100ms） |
...
| FR-08-A4 | Lambda handler状態汚染なしテスト | 同一handler関数を2回連続呼び出し → 2回目のレスポンスが1回目と同じ正確さであること（状態汚染なし）。R4ブラックボックス原則準拠 — 内部グローバル変数ではなく外部入出力契約で検証 |
...
| FR-08-A7 | Lambda Layer依存解決テスト | Layer内パッケージ（strands-agents, slack-bolt等）が正しくimport可能なことをstub環境で検証。import順序・パス解決のテスト |
| FR-08-A8 | Lambda handler ログ出力テスト | handler実行時にstructured log（JSON形式、request_id含む）が正しく出力されることをstdout captureで検証（R16 Observability） |
| FR-08-A9 | Lambda handler エラーハンドリングテスト | Bedrock APIタイムアウト/Slack API 429 (rate limit)/Lambda実行時間上限接近時のgraceful degradation。R17障害シナリオ準拠 |

#### FR-08-B: インテグレーションテスト（実AWS — CI integrationジョブ）

| ID | 要求 | 受入基準 |
|---|---|---|
| FR-08-B1 | CFnスタックデプロイ+検証+クリーンアップ | テスト用CFnテンプレートでLambda+API GWスタック作成→リソース存在確認→スタック削除。CI Secrets使用。※複数AWSサービス結合だがCI自動実行可能なためintegration分類（E2Eは手動/ステージング環境前提のフルフロー検証に限定） |
...
| FR-08-B6 | Lambda Layer デプロイテスト | Layerアップロード→Lambda関数にアタッチ→Layer内パッケージのimport成功確認→Layer削除 |
| FR-08-B7 | CloudWatch Logs検証 | Lambda invoke後にCloudWatch Logsにstructured log（JSON形式、request_id含む）が出力されていることを確認 |
...
| NFR-01-3 | 🆕 CI cloud-integration実行時間 | ≤ 15分（Phase 3b初回計測後に再調整。CFnスタック作成3-5分+テスト1-2分+削除3-5分を想定） | CFn create/deleteの実測値に基づき調整 |
| NFR-01-4 | 🆕 Lambda冷起動時間 | ≤ 10秒。E2Eテストで計測しCI artifactに記録。超過時はテスト失敗（`assert cold_start_ms <= 10000`） | Lambda運用の応答性指標 |
...
> **注記:** 本specのPhase番号（0〜4）はテストインフラspec独自の連番であり、yui-agent本体のPhase番号（Phase 0-3: ローカル実装、Phase 6: Lambda Deployment等）とは別体系。Section 8で両者の依存関係を明示。
...
| 3a | 🆕 クラウドコンポーネントテスト — Lambda handler/Secrets Manager/Events API/Layer/ログのmockテスト | 2.5日 | Kiro | FR-08-A全項。`pytest -m component` に含まれてCIで毎回実行。仕様変更バッファ0.5日込み |
...
|---|---|---|---|
| `test_lambda_handler.py` | Lambda handler — Events APIパース、challenge応答、イベントルーティング、エラーハンドリング、ログ出力 | component | 20-25 |
...
| `test_lambda_layers.py` | Lambda Layer依存解決 — import可能性、パス解決 | component | 4-6 |

**テスト名パターン例（R1: 3パーツ構成）:**
```
test_lambda_handler__slack_challenge_event__returns_challenge_value
test_lambda_handler__invalid_signature__returns_401_unauthorized
test_lambda_handler__bedrock_timeout__returns_graceful_error_and_logs
test_lambda_handler__consecutive_invocations__no_state_pollution
test_secrets_manager__secret_not_found__raises_configuration_error
test_events_api_adapter__socket_mode_config__selects_socket_adapter
test_eventbridge_handler__schedule_event__triggers_heartbeat
```
...
| `test_lambda_layers_live.py` | Lambda Layer実デプロイ→アタッチ→import確認→削除 | integration | 3-4 |
| `test_cloudwatch_logs.py` | Lambda invoke→CloudWatch Logsにstructured log出力確認 | integration | 2-3 |
...
| 🆕 Lambda handler未実装でテスト先行困難 | 高 | Phase 3aブロック | Lambda handler実装（Phase 6）と並行。テストは外部インターフェース（Slack Events API形式の入出力）のみ検証。内部ルーティングロジックのテストはPhase 6 design.md確定後に追加。0.5日バッファ込み（仕様変更時の書き直し対応） |
| 🆕 CFnデプロイに時間がかかりCI超過 | 中 | Phase 3b/NFR-01-3 | テスト用は最小構成（Lambda 1つ+API GW 1つ）。テスト後即削除 |
| 🆕 E2Eテストの不安定性（Slack API latency等） | 高 | Phase 3c | テストコード内にリトライロジック埋め込み（最大3回、exponential backoff）。`@pytest.mark.flaky` は使用しない — AGENTS.md「逃げをなくす」精神に準拠。E2Eテストのみ `assert` 前にポーリング+タイムアウト余裕（30秒）で安定化 |
| 🆕 AWSデプロイ権限不足 | 中 | Phase 3b | hanさんにデプロイ権限依頼。IAM最小権限運用のためCI用IAMロール作成が必要 |
| 🆕 Lambda パッケージサイズ超過 | 中 | Phase 3b | 50MB zip / 250MB unzipped制限。Layer分割で依存を分離。CIで`aws lambda get-function`によるCodeSizeチェック追加 |
| 🆕 Lambda実行時間上限とBedrock長時間推論の衝突 | 中 | Phase 3a/3c | Lambda 15分上限とBedrock推論時間のバランス。handler内にタイムアウトガード（残り30秒でgraceful shutdown）を実装しテストで検証 |
